...
app.add_middleware(
    SessionMiddleware,
    secret_key="rock-ai-super-secret-key-change-this-in-production"
)
...
def index(request: Request):
    ...
    if db_user["status"] == "rejected":
        ...
        return HTMLResponse(
            ...
            "<h1>Access Restricted</h1><p style='color:#94a3b8;'>Your access request was not approved by the founder (Pavan Kalyan).</p>"
    ...
    suggestion = request.session.pop("suggestion", None)
...
def add_experiment(
    ...
    target_substrate_distance_mm: float = Form(...),
    sputtering_time_min: float = Form(...),
    film_thickness_nm: float = Form(None),
    deposition_rate_nm_min: float = Form(None),
