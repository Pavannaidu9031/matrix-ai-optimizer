...
async def auth_callback(request: Request):
    ...
    request.session["user"] = {"id": db_user["id"], "email": db_user["email"], "name": db_uimport io
import os
import smtplib
import sqlite3
from email.mime.multipart import MIMEMultipart
...
def index(request: Request):
    ...
    if not user_session:
        return templates.TemplateResponse(
            request, "index.html", {"user": None, "experiments": [], "stats": {}}
        )
    ...
    if db_user["status"] == "pending":
        return templates.TemplateResponse(
            request, "pending.html", {"user": db_user}
        )
    ...
    raw_experiments = database.get_experiments_by_user(db_user["id"])
    experiments = (
        [dict(row) for row in raw_experiments] if raw_experiments else []
    )

    total_runs = len(experiments)
    monoclinic_runs = sum(
        1 for e in experiments if e.get("xrd_phase") == "Monoclinic"
    )
    shifts = [
        float(e["wavelength_shift_pm"])
        for e in experiments
        if e.get("wavelength_shift_pm") is not None
    ]
    best_shift = max(shifts) if shifts else 0.0
    runs_remaining = max(0, 25 - total_runs)
    ...
    return templates.TemplateResponse(
        request,
        "index.html",
        {"user": db_user, "experiments": experiments, "stats": stats},
    )
...
async def get_bayes_suggestion(request: Request):
    ...
    try:
        ...
        recent_suggestions = database.get_recent_suggestions(
            user_session["id"], limit=3
        )

        result = optimizer.generate_bayesian_suggestion(
            experiments, recent_suggestions
        )

        # Save suggestion to history
        database.save_suggestion_history(
            user_session["id"], user_session["email"], result
        )
        ...
        return JSONResponse(
            status_code=500,
            content={"message": f"Optimization engine error: {str(e)}"},
        )
...
def export_csv(request: Request):
    ...
    response.headers["Content-Disposition"] = (
        "attachment; filename=MatrixAI_Thin_Film_Experiments.csv"
    )
    return response
...
async def auth_callback(request: Request):
    ...
    db_user = database.get_or_create_user(
        user_info, founder_email=FOUNDER_EMAIL
    )
...
def logout(request: Request):
    request.session.clear()
    return RedirectResponse("/")ser["name"]}
    return RedirectResponse("/")
